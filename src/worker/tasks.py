...
def pipeline_run(self, fetch_limit: int | None = None, dry_run: bool = False):
    ...
    try:
            ...
            try:
                ...
                with sqlite3.connect(article_repo.db_path) as conn:
                    conn.execute(
                        ...
                        (a.id, a.url, a.title, a.source, a.content, a.fetched_at.isoformat() if a.fetched_at else None, 0),
                    )
            ...
            try:
                ...
                with sqlite3.connect(summary_repo.db_path) as conn:
                    conn.execute(
                        ...
                        (s.id, s.article_id, s.summary_text, s.model_used, s.created_at.isoformat() if s.created_at else None),
                    )
            except Exception:
                pass
        ...
        _finish(run_id, "success", articles_fetched=len(articles), summaries_generated=len(summaries), telegram_sent=telegram_sent)
        return {"status": "success", "run_id": run_id, "articles_fetched": len(articles), "summaries_generated": len(summaries)}
