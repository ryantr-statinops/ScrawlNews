...
export function DigestPanel({ digests, onSelect, loading = false, error }: { digests: Digest[]; onSelect?: (digest: Digest) => void; loading?: boolean; error?: string }) {
  ...
  return <Card withBorder className="digest-panel" style={{ maxHeight: "70vh", overflowY: "auto" }}>
    ...
    {!loading && !error && latestDigests.length > 0 ? <SimpleGrid cols={1}>{latestDigests.map((digest) => <Card key={digest.id} withBorder shadow="xs" onClick={() => selectDigest(digest)} tabIndex={onSelect ? 0 : undefined} role={onSelect ? "button" : undefined} aria-label={onSelect ? `Open digest: ${digest.title}` : undefined} onKeyDown={(event) => { if (onSelect && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); selectDigest(digest); } }} style={{ cursor: onSelect ? "pointer" : undefined }}><Group justify="space-between" gap="xs"><Text fw={600}>{digest.title}</Text>{digest.status !== "success" ? <Badge color="red">{digest.status}</Badge> : null}</Group><Text size="sm" c="dimmed" mb="xs">{digest.category} · {digest.article_count} articles</Text>{digest.error ? <Text size="xs" c="red" mb="xs">{digest.error}</Text> : null}<div className="digest-panel__preview"><MarkdownContent content={digest.digest_text || "No digest content available."} /></div>{onSelect ? <Button variant="subtle" size="compact-sm" mt="sm" onClick={(event) => { event.stopPropagation(); selectDigest(digest); }}>Open digest</Button> : null}</Card>)}</SimpleGrid> : null}
  </Card>;
}
