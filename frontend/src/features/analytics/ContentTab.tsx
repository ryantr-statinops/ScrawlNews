...
export function ContentTab({ filters }: { filters: AnalyticsFiltersState }) {
  ...
  return (
    <>
      ...
      <Card withBorder mb="md" role="button" tabIndex={0} aria-label="Inspect content timeline" onKeyDown={(event) => { if (event.key === "Enter") setSelection({ title: "Content timeline", kind: "articles", href: "/", hrefLabel: "Open Feed" }); }} onClick={() => setSelection({ title: "Content timeline", kind: "articles", href: "/", hrefLabel: "Open Feed" })} style={{ cursor: "pointer" }}>
      ...
      <SimpleGrid cols={{ base: 1, xl: 2 }}>
        <Card withBorder>
          ...
          <Table.ScrollContainer minWidth={520}><Table striped highlightOnHover>
